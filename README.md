...
每段都有本地 SVG 徽章。最高段位不仅要求综合分达到 92，还要求“废话鉴别”和“人话翻译”同时过线：会说复杂话不是终点，能选择说清楚才是。

### 4. 成长牌局：任务与晋级闭环

- 最新测评段位作为成长基线，测评结果与成长段位分开保存
- 每次推荐 3 个任务，优先补齐五维中的弱项
- 挑战题首答正确获得 35 XP；真实场景练习由用户自证，获得 20 XP
- 每 100 XP 晋升一个段位，溢出经验保留，重复任务不能反复结算
- 晋级时触发专属翻牌动效并解锁新段位徽章
- 内置 15 个任务，覆盖术语、语境、梗文化、废话过滤和人话翻译
...
- 牌桌打法、招牌动作、隐藏底牌、错误下注、反制牌和段位缺口
- 报告内直接给出三项晋级任务，并可进入成长牌局
- 第二次测评起展示五维变化对比
...
- 正文按段位、徽章、强弱维度和晋级缺口动态生成，约 500–700 字，内含贴合产品的黑话玩梗
- 普通浏览器没有 Native 桥接时保留图片预览，不阻断报告浏览

## 视觉与产品哲思

视觉采用“德州扑克俱乐部 × 职场编辑部”：深绿绒面牌桌、牛血红、黄铜线、象牙纸、筹码与段位徽章共同形成产品识别度。首页中央是四人打牌场景，发牌、筹码呼吸和晋级翻牌使用克制动效。页面没有蓝紫渐变、霓虹、玻璃拟态、机器人或通用仪表盘卡片。

中文排版按 `chinese-font-selector` 的场景分层处理：标题使用本地系统高质量黑体栈保证强识别与离线稳定；正文使用系统无衬线保证小屏可读；判词和情绪文案使用楷体/仿宋回退栈形成编辑感。没有外链字体，也不会因字体下载失败阻塞页面。

产品设计吸收了几类成熟思路，但不照搬外壳：

- **Steve Jobs 的减法**：主线只围绕“听懂—拆解—说清”展开，附加模块服务复用，不堆无关功能。
- **Don Norman 的可见反馈**：答题选中态、即时反馈、真实阶段、动态题量和报告加载都有明确状态。
- **游戏化晋级**：段位线、专项路线、Boss、晋级赛与 XP 成长任务组成完整闭环，而不是一次性算分。
...
│   ├── question-bank.js    # 68 道题及五维权重
│   ├── content-data.js     # 八段位完整档案、36 词词典、维度文案
│   ├── growth-core.js      # XP、晋级、推荐与重复结算保护
│   ├── growth-data.js      # 15 个成长任务
│   ├── rank-assets.js      # 八段 SVG 映射
│   ├── ranks/              # 八枚本地段位徽章
│   ├── storage.js          # 本地历史、成长状态、去重、对比
...
```powershell
node --test .\tests\quiz-core.test.js .\tests\content-data.test.js .\tests\storage.test.js .\tests\poster.test.js .\tests\growth-core.test.js .\tests\rank-assets.test.js
```
...
```text
ZIP 压缩后：62,866 bytes
解压运行文件：198,742 bytes
运行条目：19
...
```text
21 项 Node 单元测试通过
9 个运行 JavaScript 文件语法检查通过
Playwright 全流程通过
320×568 / 375×812 / 430×932 无横向溢出
1080×1440 海报生成通过
相册保存与笔记发布桥接模拟通过
固定顶端黄铜线滚动前后均保持 `top: 0`
成长任务、重复结算保护与 XP 溢出晋级通过
UPLOAD_PREFLIGHT=PASS
ZIP PATH TEST PASSED
```
